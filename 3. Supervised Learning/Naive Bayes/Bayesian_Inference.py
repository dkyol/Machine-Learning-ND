...
#if run multiple times values cycle to NAN, re-run block above to reset values 
...
Solution:
'''
sans_punctuation_documents = ['none'] * len(lower_case_documents)

pun = '?!.,'
punOut ='    '
tab = "".maketrans(pun, punOut)

for i in range(len(lower_case_documents)):
    sans_punctuation_documents[i] = lower_case_documents[i].translate(tab)
...
for i in range(len(preprocessed_documents)):
    for j in preprocessed_documents[i]:
        in_list.append(j)
        

frequency_list = Counter(in_list)
...
frequency_matrix = pd.DataFrame(doc_array, columns = count_vector.get_feature_names(), index = ['1', '2', '3', '4'])
frequency_matrix
